...
@interface ImagePickerManager ()<CLImageEditorDelegate, CLImageEditorTransitionDelegate, CLImageEditorThemeDelegate>
...
- (void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
    ...
    if ([mediaType isEqualToString:(NSString *)kUTTypeImage]) { // PHOTOS
        ...
        if (imageURL && [[imageURL absoluteString] rangeOfString:@"ext=GIF"].location != NSNotFound) {
            self.callback(@[@{@"error": @"git is not supported"}]);
            return;
        }
        [self showImageEditor:image formPicker:picker];
    } else { //不支持VIDEO
        self.callback(@[@{@"error": @"video is not supported"}]);
        return;
    }
}
- (void)deperate_imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
 ...
 * 显示图片编辑器
 */
- (void)showImageEditor:(UIImage *)image formPicker:(UIImagePickerController *)picker
{
...
- (void)imageEditor:(CLImageEditor*)editor didFinishEdittingWithImage:(UIImage*)image
{
    ...
    dispatch_block_t dismissCompletionBlock = ^{
        ...
        UIImage *image = [self fixOrientation:originImage];
